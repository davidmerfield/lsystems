...
DRAW_ODIR=$(ODIR)/$(DRAW)
DRAW_SRCS=$(wildcard $(DRAW_SRCDIR)/*.cc)
DRAW_OBJS=$(patsubst $(DRAW_SRCDIR)/%.cc,$(DRAW_ODIR)/%.o,$(DRAW_SRCS))
...
LSYSTEM_SRCS=$(wildcard $(LSYSTEM_SRCDIR)/*.cc)
LSYSTEM_OBJS=$(patsubst $(LSYSTEM_SRCDIR)/%.cc,$(LSYSTEM_ODIR)/%.o,$(LSYSTEM_SRCS))
...
$(LSYSTEM_ODIR)/%.o: $(LSYSTEM_SRCDIR)/%.cc $(LSYSTEM_ODIR)
	$(CC) -c -o $@ $(filter %.cc,$<) $(LSYSTEM_CFLAGS)

$(DRAW_ODIR)/%.o: $(DRAW_SRCDIR)/%.cc $(DRAW_ODIR)
	$(CC) -c -o $@ $(filter %.cc,$<) $(DRAW_CFLAGS)
